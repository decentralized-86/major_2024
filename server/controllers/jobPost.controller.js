...
const oneJobPost = async (req,res) => {
    ...
    try {
        ...
        if(!job)
        {
            return res.status(404).json({message: `No Job Post with id ${company_id}`})
        }
...
const allAppliedStudents = async (req,res) => {
    ...
    try {
        ...
        if(result.length === 0)
        {
            return res.status(404).json({sucess: true, message: `No Job Post found with id ${company_id}`})
        }
...
module.exports = {addJobPost, allJobPosts, oneJobPost,updateJobPost,deleteJobPost,allAppliedStudents};